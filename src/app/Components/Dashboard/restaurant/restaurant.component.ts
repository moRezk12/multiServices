...
export class RestaurantComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.restaurantForm = this.fb.group({
      ...
      phone: ['', [Validators.required, Validators.pattern(/^\+20[0-9]{9}$/)]], // +20 + 9 أرقام
...
removeMenuImage(index: number) {
  this.menuImages.removeAt(index); // شيل من الـ FormArray
  this.menuImagePreviews.splice(index, 1); // شيل من الـ preview
}
  ...
  openEditModal(restaurant: any) {
    ...
    if (restaurant.menuImages) {
      restaurant.menuImages.forEach((img: any) => {
        this.menuImages.push(this.fb.control(img)); // Store in form
        this.menuImagePreviews.push(img.secure_url); // Display link directly
      });
    }
